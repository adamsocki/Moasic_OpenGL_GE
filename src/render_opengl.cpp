...
void ScaleSpriteByNumber(vec2 position, vec2 scale, real32 angle, Sprite* texture, float numberOfTimes) {
    Shader* shader = &Game->ScaleSpriteByNumber;
    SetShader(shader);
    ...
    glUniform1fv(shader->uniforms[3].id, 1, &numberOfTimes);
...
void SpriteSheetShader(vec2 position, vec2 scale, real32 angle, Sprite* texture, vec2 indexPos, vec2 pos2) {
    Shader* shader = &Game->SpriteSheetShader    ;
    SetShader(shader);
    //float spriteWidth =
    glEnable(GL_BLEND);
    glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
   // vec2 *textureSize = &V2(texture->width, texture->height);
   ...
   // mesh->vertCount *= 22;
    ...
    glActiveTexture(GL_TEXTURE0);
    glBindTexture(GL_TEXTURE_2D, texture->textureID);
    glUniform1fv(shader->uniforms[2].id, 1, &indexPos.x);
    glUniform1fv(shader->uniforms[3].id, 1, &indexPos.y);
    glUniform1fv(shader->uniforms[4].id, 1, &indexPos.x);
    glUniform1fv(shader->uniforms[5].id, 1, &indexPos.x);
...
void DrawSpriteInvertedColors(vec2 position, vec2 scale, real32 angle, Sprite* texture) {
    Shader* shader = &Game->invertedColorsQuadShader;
    SetShader(shader);
...
void DrawPerlinBlendTwo(vec2 position, vec2 scale, real32 angle, Sprite* texture0, Sprite* texture1) {
    Shader* shader = &Game->perlinMixShader;
    SetShader(shader);
...
void DrawSpriteBlendTwo(vec2 position, vec2 scale, real32 angle, Sprite* texture0, Sprite* texture1) {
    Shader* shader = &Game->blendTwoShader;
    SetShader(shader);
